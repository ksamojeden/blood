 <?php
	class chart {
		...
		public function calculateMaxMinValue() {
			...
			$i = 1;
			$this->maxVal1 = $this->y1[0];
			$this->minVal1 = $this->y1[0];
			for ($i; $i < $N; $i++) {
			...
			$i = 1;
			$this->maxVal2 = $this->y2[0];
			$this->minVal2 = $this->y2[0];
			for ($i; $i < $N; $i++) {
		...
		public function calculatePoints() {
			...
			for ($i = 0; $i < $N; $i++) {
				...
				$xValue = $i / ($N-1) * $this->chartWidth + $xPos;
		...
		public function calcXLabelPos() { 
			$N = count($this->y1);
			$xPos = 50;
			$this->xLabelPos[0] = $xPos;
			$this->xLabelPos[$N-1] = $this->chartWidth - 45 + $xPos;			
			for ($i=1; $i<$N-1; $i++) {
				$this->xLabelPos[$i] = $i * $this->chartWidth / ($N-1) + $xPos - 15;
			}
		}
